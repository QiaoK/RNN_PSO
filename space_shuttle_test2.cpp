...
void test_result(NeuralNetwork* network,std::vector<std::vector<DTYPE>*> *inputs,std::vector<std::vector<DTYPE>*> *outputs){
	...
	FILE *ptr_myfile=fopen("space_shuttle_training2.csv","w");
	FILE *error=fopen("space_shuttle_error2.csv","w");
	DTYPE square_error;
	...
	fclose(error);
	printf("\n");
	ptr_myfile=fopen("space_shuttle_target2.csv","w");
...
//62316
	char filename[200];
	DWORD particle_num=5041,epochs=10,input_nodes=1,output_nodes=1,hidden_nodes=40,seed=5332,gradient_list_size=31,mse_list_size=10,i;
	DTYPE C1=2,C2=2,alpha=0.729,bound=.8872983;
	DTYPE learning_rate=.05;
	ACTIVATION_TYPE hidden_type=SIGMOID,output_type=SIGMOID;

	std::vector<std::vector<DTYPE>*>* shuttle_data= read_csv("space_shuttle_puppet_pulled_off.txt",SPACE_SHUTTLE,FALSE);
...
*/
	...
	shuttle_data=sax_encoding(shuttle_data,128);
	for(i=0;i<shuttle_data->size();i++){
		//printf("%lf\n",shuttle_data[0][i][0][0]);
	}
...
/*
	...
	for(i=0;i<networks->size();i++){
		sprintf(filename,"space_shuttle_puppet_pulled_off%lld.bin",i);
...
/*
	networks->resize(11);
	for(i=0;i<networks->size();i++){
		sprintf(filename,"space_shuttle_puppet_pulled_off_refined%lld.bin",i);
	...
	gettimeofday(&start,NULL);
	#pragma omp parallel private(i,pre_score,pre_mse,mse,score) num_threads(32)
	{
...
*/
	...
	for(j=0;j<n_mse->size();j++){
		...
		char filename2[200];
		sprintf(filename2,"space_shuttle2_adam_curve_benchmark1-%lld.csv",j);
...
/*
	data->clear();
	data->resize(3000);
	std::copy(shuttle_data->begin(), shuttle_data->begin()+3000, data->begin());
...
*/
	...
	//test_result(networks[0][2], test_inputs, test_outputs);
